...
contract FractionCalculatorTest is Test {
    ...
    function test_DivisibleBy10_0_015Ether() public view {
        uint256 amount = 0.015 ether; // 0.015 ETH = 15 * 10^15 wei - Actually IS divisible by 10

        assertTrue(calculator.isDivisibleBy10(amount), "0.015 ether should be divisible by 10");

        (uint256 thirty, uint256 ten, uint256 remaining,) = calculator.splitAmount(amount);

        // 30% of 0.015 ETH = 0.0045 ETH = 4.5 * 10^15 wei (exact)
        assertEq(thirty, 45 * 10 ** 14, "30% should be 4.5 * 10^15 wei (0.0045 ETH)");

        // 10% of 0.015 ETH = 0.0015 ETH = 1.5 * 10^15 wei (exact)
        assertEq(ten, 15 * 10 ** 14, "10% should be 1.5 * 10^15 wei (0.0015 ETH)");
